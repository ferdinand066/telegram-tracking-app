...
import { handleIncomeCommand, handleExpenseCommand } from "~/usecase/bot/handle-transaction-command";
...
bot.use(async (ctx, next) => {
  ctx.user = ctx.from
    ? await userRepository.upsert(ctx.from.id, ctx.from.username, ctx.from.first_name)
    : null;
  return next();
});
